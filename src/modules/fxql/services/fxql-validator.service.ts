  ...
  async validateCurrencyPair(
    ...
    if (!source || source !== source.toUpperCase()) {
      throw new InvalidCurrencyError(
        `Invalid currency case: '${source}' should be '${source.toUpperCase()}'`,
        line,
        column,
      );
    }
    ...
    if (
      !VALID_CURRENCIES.includes(source as any) ||
      !VALID_CURRENCIES.includes(destination as any)
    ) {
      throw new InvalidCurrencyError(
        `Invalid currency pair: ${source}-${destination}`,
        line,
        column,
      );
    }
  ...
  async validateAmount(
    ...
    if (isNaN(amount)) {
      throw new InvalidAmountError(
        `Invalid amount: '${value}' is not a valid numeric amount`,
        line,
        column,
      );
    }
  ...
  async validateCap(cap: number, line: number, column: number): Promise<void> {
    if (!Number.isInteger(cap) || cap < 0) {
      throw new InvalidCapError(
        `Invalid cap: ${cap}  shop be a positive number`,
        line,
        column,
      );
    }
