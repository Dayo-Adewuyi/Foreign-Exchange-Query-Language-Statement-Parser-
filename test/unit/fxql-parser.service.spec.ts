  ...
  describe('parse', () => {
    ...
    const errorCases = [
      {
        ...
        pattern:
          'Invalid currency pair format: "usd-GBP { BUY 0.85 SELL 0.90 CAP 10000 }"',
      },
      {
        name: 'missing fields',
        ...
        error: InvalidCurrencyError,
        pattern:
          'Invalid currency pair format: "USD-GBP { BUY 0.85 SELL 0.90 }"',
      },
      {
    ...
    errorCases.forEach(({ name, input, error, pattern }) => {
      it(`throws on ${name}`, async () => {
        await expect(service.parse(input)).rejects.toThrow(error);
        await expect(service.parse(input)).rejects.toThrow(pattern);
      });
    });

    it('throws on exceeding rate limit', async () => {
      const pairs = Array(1001).fill(validFxql).join('\n\n');
      await expect(service.parse(pairs)).rejects.toThrow(InvalidSyntaxError);
    });
  });
});
